import importlib
import json
import os
import shutil


class Jsonable:
    ext = ''

    def __init__(self, **kwargs):
        self._kwargs = kwargs

    @property
    def kwargs(self):
        return self._kwargs

    @property
    def data(self):
        raise NotImplementedError

    @data.setter
    def data(self, value):
        raise NotImplementedError

    def _list_to_serialized(self, l_):
        out = []
        v = {}
        for d in l_:
            if isinstance(d, list):
                out.append(self._list_to_serialized(d))
            elif isinstance(d, Jsonable):
                defn, tmp = d.to_dict()
                out.append(defn)
                v.update(tmp)
            else:
                out.append(d)
        return out, v

    @staticmethod
    def _parse_list(l_, v):
        out = []
        for d in l_:
            if isinstance(d, dict) and \
                    {'id', 'kwargs', 'cls', 'module'}.issubset(d):
                out.append(Jsonable.from_dict(d, v))
            elif isinstance(d, list):
                out.append(Jsonable._parse_list(d, v))
            else:
                out.append(d)
        return out

    def to_dict(self):
        values = {}
        kwargs = self.kwargs.copy()
        for k, d in kwargs.items():
            if isinstance(d, Jsonable):
                kwargs[k], tmp = d.to_dict()
                values.update(tmp)
            elif isinstance(d, list):
                kwargs[k], tmp = self._list_to_serialized(d)
                values.update(tmp)
        out = {
            'module': self.__module__,
            'cls': self.__class__.__name__,
            'id': str(id(self)),
            'kwargs': kwargs,
        }
        if out['module'] == '__main__':
            raise Exception('Can\'t save a class in "__main__"')
        try:
            values[out['id']] = {'data': self.data, 'ext': self.ext}
        except NotImplementedError:
            pass
        return out, values

    def save(self, path, force=False):
        if os.path.exists(path) and not force:
            raise Exception(f'File/ directory {path} already exists! Set force=True'
                            'if you\'d like to go ahead anyway.')
        if force:
            shutil.rmtree(path)
        os.makedirs(path)
        d, v = self.to_dict()
        with open(path + '/ai.json', 'w') as f:
            json.dump(d, f, indent=2)
        for id_ in v:
            with open(f'{path}/{id_}{v[id_]["ext"]}', 'wb') as f:
                f.write(v[id_]['data'])

    @staticmethod
    def load(path):
        with open(path + '/ai.json') as f:
            d = json.load(f)
        v = {}
        for file in os.listdir(path):
            id_ = file.split('.')[0]
            if id_ == 'ai':
                continue
            with open(path + '/' + file, 'rb') as f:
                data = f.read()
            v[id_] = {'data': data}
        return Jsonable.from_dict(d, v)

    @staticmethod
    def from_dict(defn, values):
        module = importlib.import_module(defn['module'])
        cls = getattr(module, defn['cls'])
        kwargs = defn['kwargs'].copy()
        for k, val in kwargs.items():
            if isinstance(val, dict) and {'cls', 'kwargs', 'id'}.issubset(set(val.keys())):
                kwargs[k] = Jsonable.from_dict(val, values)
            elif isinstance(val, list):
                kwargs[k] = Jsonable._parse_list(val, values)
        instance = cls(**kwargs)
        if defn['id'] in values:
            instance.data = values[defn['id']]['data']
        return instance

